...
{
	public static void Expose(PropertyField field, Rect position)
	{
		if (field.Type == SerializedPropertyType.Integer)
		{
			var oldValue = (int)field.GetValue();
			var newValue = EditorGUI.IntField(position,oldValue);
		...
		else if (field.Type == SerializedPropertyType.Float)
		{
			...
			var newValue = EditorGUI.FloatField(position,oldValue);
			if (oldValue != newValue)
				field.SetValue(newValue);
		}
		else if (field.Type == SerializedPropertyType.Boolean)
		{
			...
			var newValue = EditorGUI.Toggle(position, oldValue);
			if (oldValue != newValue)
				field.SetValue(newValue);
		}
		else if (field.Type == SerializedPropertyType.String)
		{
			...
			var newValue = EditorGUI.TextField(position, oldValue);
			if (oldValue != newValue)
				field.SetValue(newValue);
		}
		else if (field.Type == SerializedPropertyType.Vector2)
		{
			...
			var newValue = EditorGUI.Vector2Field(position, field.Name, oldValue);
			if (oldValue != newValue)
				field.SetValue(newValue);
		}
		else if (field.Type == SerializedPropertyType.Vector3)
		{
			...
			var newValue = EditorGUI.Vector3Field(position, field.Name, oldValue);
			if (oldValue != newValue)
				field.SetValue(newValue);
		}
		else if (field.Type == SerializedPropertyType.Enum)
		{
			...
			var newValue = EditorGUI.EnumPopup(position, oldValue);
			if (oldValue != newValue)
				field.SetValue(newValue);
		}
	}
